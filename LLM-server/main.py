...
def construct_prompt(content: str, state: RequestState) -> str:
    ...
    # elif prompt_type == "testsummary":
    #     constructed_prompt = (
    #         "Please generate a summary of the unit test given between the [CODE] and [/CODE] tags. Place your suggested summary between the [TESTSUMMARY] and [/TESTSUMMARY] tags\n"
    #         f"[CODE]\n{content}\n[/CODE]\n\n"
    #     )
    elif prompt_type == "testsummary":
        constructed_prompt = (
            "[INST] As a detail-oriented developer focused on enhancing the clarity of a test suite, " 
            "your task is to analyze the provided Java code and generate a"
            "descriptive and concise summary of the unit test. Follow these steps:\n"
            "1. Carefully read the Java code between the [CODE] tags.\n"
            "2. Identify the primary functionality or purpose of the test.\n"
            "3. Analyze actions performed in the test.\n"
            "4. Formulate a test method summary that succinctly and concisely captures test case"
            "which includes the key action or feature tested and the main operations performed.\n"
            "5. Place your suggested test summary between the [TESTSUMMARY] and [/TESTSUMMARY] tags, ensuring it is "
            "clear and precise without additional descriptions.\n"
            "Remember, your focus is on clarity and precision. Use your expertise to provide a meaningful and "
...
def utilize_llm_openai(test: str, state: RequestState, model: str = "gpt-3.5-turbo") -> str:
    """
    A function to utilize an LLM available in OpenAI by providing the prompt and the (optimal) model to use.
    :param test: the test code to analyze
    :param state: the current state of the request
    :param model: the model to utilize, set to 'gpt-3.5-turbo' by default
    ...
    try:
        response = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": "You are a Java developer optimizing JUnit tests for clarity."},
                {"role": "user", "content": "[INST] As a detail-oriented developer focused on enhancing the clarity of a test suite, "
                                            "your task is to analyze the provided Java code and generate a "
                                            "descriptive and concise summary of the unit test. Follow these steps:\n"
                                            "1. Carefully read the Java code between the [CODE] tags.\n"
                                            "2. Identify the primary functionality or purpose of the test.\n"
                                            "3. Analyze actions performed in the test.\n"
                                            "4. Formulate a test method summary that succinctly and concisely captures the test case "
                                            "which includes the key action or feature tested and the main operations performed.\n"
                                            "5. Place your suggested test summary between the [TESTSUMMARY] and [/TESTSUMMARY] tags, ensuring it is "
                                            "clear and precise without additional descriptions.\n"
                                            "Remember, your focus is on clarity and precision. Use your expertise to provide a meaningful and "
...
def utilize_llm(prompt: str, state: RequestState, model: str = "codellama:7b-instruct") -> str:
    ...
    if response.status_code == 200:
        ...
        return utilize_llm_openai(prompt, state, model=model)
...
def parse_test_summary(extracted_answer: str) -> str:
    # Define a pattern that matches the specified phrases or tags anywhere in the string
    pattern = r'(Test Method Summary:|Test Summary:|Here is a summary of the test case:|Here is a summary of the test method:|Here\'s a test summary for the above Java code:|Based on the code you provided, here is a summary of the test unit:|Here is a suggested test summary for the provided Java code:|\[TESTSUMMARY\]|\[/TESTSUMMARY\])'
    print("\n\nA) The extracted answer in the method is:\n" + extracted_answer)
    # Remove the specified phrases or tags from the string
    extracted_answer = re.sub(pattern, '', extracted_answer, flags=re.IGNORECASE)
    print("\n\nB) The extracted answer after subbing it is:\n" + extracted_answer)
    # Define a pattern to capture content within triple backticks
    code_block_pattern = r'```(.*?)```'

    # Find all occurrences of the code block pattern
    code_blocks = re.findall(code_block_pattern, extracted_answer, flags=re.DOTALL)
    
    # Check if code blocks were found
    if code_blocks:
        print("\n\nC) The code block is:\n" + str(code_blocks))
        # Join all the code blocks into a single string, separated by newlines
        code_block_content = '\n'.join(code_blocks)
        print("\n\nD) The code block content is:\n" + code_block_content)

        # Strip leading and trailing whitespace
        code_block_content = code_block_content.strip()
        print("\n\nE) The code block content after stripping is:\n" + code_block_content)
        return code_block_content
    else:
        # No code blocks found, return the original extracted_answer
...
def extract_answer(response: str, prompt_type: str, original_code: Optional[str] = None,
    ...
    if prompt_type == "testname":
        ...
        regex_test = r'\[TESTSUMMARY](.*?)\[/TESTSUMMARY]'
    else:
    ...
    try:
        ...
        # If the tags are not found and the prompt type is "testsummary", use the entire response
        if prompt_type == "testsummary":
            ...
            # Remove "java" if it appears at the beginning
    ...
    if prompt_type == "testname":
        ...
        extracted_answer = parse_test_summary(extracted_answer)
    else:
...
def get_llm_response(prompt: Prompt, state: RequestState) -> Response:
    ...
    if prompt.prompt_type not in ["testname", "testdata", "testsummary"] and state.get_iteration() > 5:
    ...
    try:
        ...
        response = utilize_llm_openai(constructed_prompt, state)
