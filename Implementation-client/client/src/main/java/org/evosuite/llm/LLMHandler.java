...
//        LoggingUtils.getEvoLogger().info("refined test data is: " + result.data.prompt.llmResponse);
    ...
    public String suggestSummary(String test){
        ApolloResponse<PromptQuery.Data> result = client.promptSuggestSummaryQuery(test).blockingGet();
        if(result.data == null) {
            LoggingUtils.getEvoLogger().info("** Null Pointer Exception");
            return null;
        }
        ...
        LoggingUtils.getEvoLogger().info("Summary is: " + result.data.prompt.llmResponse);
